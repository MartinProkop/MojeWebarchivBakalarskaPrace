...
Pou¾ití tohoto nástroje bylo nutné kvùli tomu, ¾e nástroje pro migraci \texttt{arc} archivù jsou psány v programovacím jazyce \texttt{Python}, ale \texttt{JHOVE2} je programován v jazyce \texttt{Java}.

Nástroj v podstatì umo¾ní spu¹tìní \texttt{Java Virtual Machine}\footnote{Webové stránky \texttt{Java} dostupné online na adrese \texttt{http://www.java.com}.} uvnitø \texttt{Python} skriptu a následnì práci s Java aplikací.

Je samozøejmé, ¾e implementace takového programu, který musí vyu¾ívat více programovacích jazykù najednou, je problematické z hlediska efektivity. Efektinìj¹í a elegantnìj¹í by samozøejmì byl nástroj, který je implementován jedním programovacím jazykem\footnote{Toto tvrzení není nutnì pravdivé. Samozøejmì existují softwarová øe¹ení, která dosahují vy¹¹í efektivity právì tím, ¾e kombinují rùzné programovací jazyky. To v¹ak není pøípad nástroje, který je potøeba implementovat pro potøeby WebArchivu}.

Ale vzhledem k tomu, ¾e je vìt¹ina vývojáøù projektù, které pracují na archivaci webu, rozsáhlá a nestejnorodá, není mo¾né tento po¾adavek splnit. Zpravdila si ka¾dý øe¹itelský tým jednodu¹e implementuje své nástroje. Pokud je pak chce jiný tým pøejmout a u¾ívat, musí se s z toho plynoucímy problémy vypoøádat.
 
Celkovì jde ale øíci, ¾e pou¾itím nástroje \texttt{Jpype} pro spojení nástrojù \texttt{warctools} a \texttt{JHOVE2} nevede k nìjakému razatnímu zeslo¾itìní techniky migrace archivu, jeliko¾ je nástroj je implementován efektivnì. Navíc v podstatì neexistuje jiná varianta, tedy mimo implementace vlastních nástrojù, co¾ by ale bylo velmi nároèné, a snad i zbyteèné, jeliko¾ napøíklad nástroj \texttt{JHOVE2} je standardní, efektivní a velmi robusní nástroj.

Nástroj je dostupný volnì ke sta¾ení na webových stránkách\footnote{Webové stránky projektu \texttt{Jpype} dostupná online na adrese \texttt{http://jpype.sourceforge.net}.} jeho vývojáøù. Jeho vyu¾ití bylo zásadní pro implementaci analýzy obsahu pøevádìných \texttt{arc} archivù. V \texttt{pøíloze AI na s. \pageref{pr:ai}} je archiv s nástrojem \texttt{Jpype}.
...
\item Zjistil jsem, ¾e nástroj havaruje, stejnì jako \texttt{WARCTOOLS / hanzo}, pøi pøevodu archivu se specifickým záznamem. 
\end{itemize}
...
\subsection{Úprava pro testování celého archivu}
Tato varianta umo¾òuje migraci \texttt{arc} archivu a zároveò provádí analýzu jeho obsahu pomocí nástroje \texttt{JHOVE2}. Jako výstup z migrace, je kromì nového \texttt{warc} archivu i soubor ve formátu \texttt{XML} vytvoøený programem \texttt{JHOVE2}, který obsahuje souhrn analýzy souborù uvniø archivu obsa¾ených.
\subsubsection{Úprava \texttt{JHOVE2}}
...
Moje nová tøída je spu¹tìna pøímo z programu \texttt{arc2warc.py}\footnote{Souèást nástroje WARCTOOLS, která je urèena k migraci archivù. Do jiných programù v rámci WARCTOOLS jsem nemusel zasahovat.} a obstará inicializaci a pøedání instrukcí nástroji \texttt{JHOVE2}, který pak provede v¹echny operace a vytvoøí výstupní soubor s informacemi o analýze obsahu \texttt{arc} archivu. Mnou implementovaná tøída umo¾òuje, aby mohl \texttt{JHOVE2} zpracovat libovolné mno¾ství souborù najednou, a vrátit pro nì souhrnný soubor s informace o analýze.
...
Ukázku této tøídy je mo¾no najít v \texttt{pøíloze AK a na s. \pageref{pr:ak}}, kompletní implementovaný nástroj je k nalezení v \texttt{pøíloze AW na s. \pageref{pr:aw}}.
\subsubsection{Úprava \texttt{WARCTOOLS / hanzo}}
...
Pùvodní program pøevádí \texttt{arc} archiv tak, ¾e pøedìlá metadata uvnitø archivu a jeho obsah -- soubory v nìm obsa¾ené -- jako \texttt{bytestream} pøevede do nového \texttt{warc} archivu. Mùj výsledný upravený program pracuje tak, ¾e bìhem pøevodu archivu provede pomocí regulárního výrazu filtrování jednotlivých souborù uvnitø archivu. Ty ulo¾í do doèasného adresáøe. Potom, co je \texttt{warc} archiv vytvoøený, spustí se \texttt{JHOVE2}, který provede analýzu v¹ech doèasnì souborù -- kopií souboru uvnitø archivu. Nakonec jsou soubory z doèasného adresáøe odstranìny.

Upravený program \texttt{arc2warc.py} pod názvem \texttt{arc2warc\-\_jhove2.py} nabízím v \texttt{pøíloze AL na s. \pageref{pr:al}}, kde jsou i moje komentáøe k úpravám zdrojového kódu.
\subsection{Úprava pro krokové testování ka¾dého souboru v archivu}
Tato varianta umo¾òuje bìhem testování obsahu archivu provádìt dal¹í operace. Pøi migraci archivu dochází k tomu, ¾e pøed tím, ne¾ nástroj \texttt{arc2warc} vlo¾í do nového \texttt{warc} archivu soubor z pùvodního archivu, provede se analýza daného souboru pomocí nástroje \texttt{JHOVE2}.
...
\subsubsection{Úprava \texttt{WARCTOOLS / hanzo}}
Opìt jsem pøi implementaci postupoval podobnì jako v první variantì implementace. Rozdíl spoèívá pouze v tom, ¾e volám nástroj \texttt{JHOVE2} z tìla skriptu \texttt{arc2warc} pøedtím, ne¾ je provedeno zaøazení souboru do nového archivu. 
...
Své programy \texttt{arc2warc\_jhove2.py} jsem otestoval pøi pøevodu nìkolika souborù \texttt{arc.gz}, které jsem pomocí nástroje \texttt{Heritrix} sám sklidil.
\subsection{Úprava pro testování celého archivu}
...
Jak se dalo pøedem oèekávat migrace spojená s testováním obsahu archivu je èasovì velmi nároèná. Pøi testování standardních 100 MB souborù \texttt{arc.gz} se èas migrace dosahoval témìø dvou minut, pøièem¾ pøevod neupraveným nástrojem -- bez pou¾ití programu \texttt{JHOVE2} -- trvá do 25 sekundy\footnote{Musím zdùraznit, ¾e jsem testování provádìl na svém osobním poèítaèi. Reálná èísla pøi pou¾ití nástroje na strojích WebArchivu by byla jistì rozdílná. Dùle¾itý je zde pouze pomìr èasu s u¾itím nástroje \texttt{JHOVE2} a bez nìj.}.
...
\subsection{Úprava pro krokové testování ka¾dého souboru v archivu}
Pro druhou variantu nástroje trvá pøevod vèetnì analýzy cca necelé dvì minuty.
\subsection{Závìr}
...
\section{Analýza havárií pøi migraci archivu}
Jak jsem zjistil, nástroje \texttt{WARCTOOLS / hanzo} i \texttt{WARCTOOLS / kpk09} havarují pøi pøevodu nìkterých archivù. Mým cílem bylo zjistit, co pøespì pùsobí havárii. Pro tento úèel jsem implementoval jednoduchou variantu migraèního nástroje, kterou nabízím v pøíloze \texttt{AR na s. \pageref{pr:ar}}.

Nástroj se pokusí o pøevod zadaného \texttt{arc.gz} souboru, pøièemz do souboru s názvem \texttt{arc2warc\_debug.log} ulo¾í informace o pøevodu. Pokud pøevod havaruje, dá se v logovacím souboru dohledat, který záznam byl pøed havárií naposledy úspì¹nì pøeveden. Podle toho pak lze jednodu¹e v indexu archivu dohledat, na kterém záznamu migrace havarovala.

V \texttt{pøíloze AS na s. \pageref{pr:as}} je k dohledání øádek z \texttt{CDX indexu}, který odpovídá souboru, na kterém se pøevod zastavil. Zde jde o \texttt{text/html} soubor, ale v jiném testovaném archivu se zastavil na \texttt{image/jpeg} souboru -- viz \texttt{pøíloha AT na s. \pageref{pr:at}}.
