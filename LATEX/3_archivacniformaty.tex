...
Øe¹itelský tým musel pøi volbì vhodného formátu poèítat se spoustou nárokù, které bude na archiv v budoucnu klást. Zejména se jednalo o po¾adavky týkající se dostupnosti ulo¾ených dat. S archivovanými daty je potøeba v krátkém èase manipulovat.

Jeliko¾ mno¾ství souborù\footnote{V posledních letech pøesahuje desítky miliónù.}, které se v rámci jednotlivých sklizní stáhne, se stále zvìt¹uje, bylo nutné jednotlivé soubory sdru¾ovat, a ne je jednodu¹e umístit do adresáøe v úlo¾i¹ti. S takto volnì ulo¾enými daty by se ¹patnì manipulovalo a i jejich archivace by nebyla snadná.
...
\item Pøi naèítání libovolného souboru z archivu je potøeba rozbalit celý archiv -- pøi 100 MB archivech jde o velkou re¾ii. Pokud ov¹em není vytvoøen index souborù uvnitø archivu, pomocí kterého se dozvíme na jaké pozici v archivu se který soubor nachází. Pak není potøeba soubor sekvenènì procházet, ale staèí pouze otevøít archiv a posunout ukazatel na správné místo. \cite{MATEJKA2006}
\end{itemize}
\subsection{U¾ití v praxi ve WebArchivu}
Právì metadat pøipojených v archivu se vyu¾ívá pøi vyhledávání souborù v databázi WebArchivu. V praxi se vytvoøí \texttt{CDX index} souboru, který obsahuje seznam souborù ulo¾ených v archivech spoleènì s metadaty a cestou k danému archivu. Tento index se vytváøí pomocí aplikace \texttt{CDX-indexer} -- souèást nástroje \texttt{Wayback}. V \texttt{pøíloze H na s. \pageref{pr:h}} nabízím ukázku \texttt{CDX indexu}.
...
\subsection{Úvod}
Pøesto¾e je formát \texttt{warc} ji¾ popsán jako \texttt{ISO standard 28500:2009}\footnote{\texttt{ISO standard} za poplatek dostupný online na adrese \texttt{http://www.iso.org/iso/iso\_catalogue/catalogue\_tc/catalogue\_de\-tail.htm?csnumber=44717}.}, je ve vìt¹inì projektù jeho pou¾ití zatím pouze testováno. Pøed tím, ne¾ se jeho pou¾ití realizuje, je potøeba podrobná analýza. Pøesto tento formát ji¾ nìkteré standardizované nástroje podporují. Napøíklad crawler \texttt{Heritrix} ji¾ umo¾òuje ukládání dat v tomto formátu.

Pou¾ití formátu \texttt{warc} se v rámci projektu WebArchiv prozatím jen plánuje. Pøed tím, ne¾ bude nasazen, je potøeba provést spoustu dílèích krokù. Je to proto, ¾e formát je prozatím je¹tì nový, a tudí¾ nebyly v¹echny testy provedeny. Pøi práci s velkými depozitáøi dat je dùle¾ité ka¾dý krok, který bude zavádìt systémovou zmìnu pøedem zvá¾it. Svou roli v tom, ¾e se o zavedení formátu uva¾uje, je ji¾ vý¹e zmínìná skuteènost: ka¾dá organizace zabývající se archivací webových zdrojù, pou¾ívá jiné nástroje a postupy. Proto ka¾dá jednotlivì øe¹í rozdílné problémy s implementací nástrojù a jejich pou¾íváním.

V následujících kapitolách pøedvedu základní vlastnosti formátu \texttt{warc} a uká¾u jeho výhody a nevýhody. Nevýhody související se zavedením formátu \texttt{warc} neznamenají, ¾e by se mìlo opustit od pøechodu na tento formát. Nevýhody spí¹e pøedstavují problémy, které doposud nebyly vyøe¹eny nebo se vùbec neøe¹ily. Pøed tím, ne¾ se pøistoupí k zavedení tohoto formátu, je potøeba se se v¹emi vyrovnat.

V \texttt{pøílohách I, J a K na s. \pageref{pr:i}} nabízím vzorové soubory typu \texttt{warc}, vydané k definici standardu. Dále v \texttt{pøílohách L, M a N na s. \pageref{pr:l}} nabízím \texttt{warc} soubory vytvoøené pomocí nástrojù pro migraci \texttt{arc} souborù, které jsem testoval. Také pøikládám výstup z programu \texttt{JHOVE2} pro jeden z \texttt{warc} archivù v \texttt{pøíloze O na s. \pageref{pr:o}}. V \texttt{pøíloze P na s. \pageref{pr:p}} je dostupný dokument IIPC zabývající se formátem. Pro úplnost uvádím i ukázku \texttt{ISO standardu warc} v \texttt{pøíloze AJ\footnote{Nejedná se o souèasný oficiální standard, ale o poslední dostupnou specifikaci z roku 2009.} na s. \pageref{pr:aj}}.
...
\end{itemize}
Soubor formátu \texttt{warc} tvoøí jednoduché zøetìzení jednoho nebo více záznamù typu \texttt{warc}. První záznam obvykle popisuje následující záznamy. Obecnì platí, ¾e obsah záznamu je buï záznam pokusu o vyhledání webového zdroje, nebo nese informace o archivovaném obsahu.

\texttt{Warc} záznamy se skládají z hlavièky, kterou následuje blok s obsahem. Hlacièka se skládá z prvního øádku, který definuje, ¾e záznam je typu warc, dále verze souboru, a flexibilní mno¾ství øádkù oddìlených symbolem \texttt{CRLF}, poté následuje ji¾ zmínìný blok obsahu. Záznam splòuje \texttt{BNF}\footnote{Backusova-Naurova forma je jedno z mo¾ných vyjádøení bezkontextové gramatiky.}, ukázka k nalezení v \texttt{pøíloze AM na s. \pageref{pr:am}}. 

Seznam definovaných polí, která se mohou v záznamu zobrazovat lze nalézt v \texttt{pøíloze AN na s. \pageref{pr:an}}. Samozøejmì nìkterá z polí jsou povinnì\footnote{Povinné: \texttt{WARC-Record-ID, Con\-tent-Length, WARC-Type}} v ka¾dém záznamu a jiné jsou nepovinné\footnote{Nepovinné: \texttt{Content-Type, WA\-RC-Block-Digest, WARC-Payload-Digest}}. Ka¾dé z polí má samozøejmì stanovená pravidla, která urèují jejich mo¾ný obsah. \cite{Information and documentation -- The WARC File Format}
...
\begin{description}
\item[Warcinfo] popisuje ostatní \texttt{warc} záznamy, které ho následují. Typicky je na zaèátku \texttt{WARC} souboru. Má tedy èistì informativní charakte. Lze v nìm najít informace o tom, jaký software daný soubor vytvoøil, \texttt{IP adresa} stroje na kterém byl vytvoøe atd. Pøíklad záznamu v \texttt{pøíloze K na s. \pageref{pr:s}}.
\item[Response] nese odpovìï na urèitý dotaz. Obsahuje informace o tom jak daný dotaz probíhal. Lze v nìm najít \texttt{HTTP} hlavièky dotazu atd.
\item[Resource] je urèitý sta¾ený zdroj, ale bez informací protokolu. Ty jsou, jak je zøejmé, neseny v jiném typu záznamu. Data jsou v nìm ulo¾eny v \texttt{Record block}. Záznam se bude li¹it podle toho, jestli bude obsahovat data z urèitého \texttt{HTTP} zdroje, \texttt{ftp} zdroje atd.
\item[Request] je ¾ádost o webový zdroj. Obsahuje detailní informace o provedeném dotazu a o ¾ádaném webovém zdroji. Pøíklad záznamu v \texttt{pøíloze J na s. \pageref{pr:j}}.
\item[Metadata] nese metadata sklizených zdrojù. Pøesnìji øeèeno obsahuje informace o libovolném sta¾eném jiném záznamu. Slou¾í tak pro reprezentaci sta¾ených dat v repozitáøích. Pøíklad záznamu lze nalézt v \texttt{pøíloze I na s. \pageref{pr:i}}.
\item[Revisit] slou¾í k znovunaètení urèitého zdroje. Dùvod proè znovunaèít urèitý zdroj mù¾e být napøíklad porovnání ulo¾ených dat se vzdálenými daty. Pomocí tohoto typu záznamu se tedy dá napøíklad o¹etøit ukládání redundantních dat.
\item[Conversion] je záznam, který obsahuje alternativní verzi ulo¾eného záznamu. Slou¾í tedy k udr¾ení informací o rozdílech mezi urèitými záznamy. Tento typ záznamu tedy podává dùle¾ité informace o verzích èi variantách ji¾ ulo¾ených záznamù.
\item[Continuation] je pokraèování jiného záznama. Tento typ záznamu se pou¾ává pokud je urèitý záznam tøeba segmentovat do více záznamù. Pøíklad záznamu typu \texttt{Continuation} v \texttt{pøíloze AO na s. \pageref{pr:ao}}.
\end{description}
...
\begin{itemize}
\item Do budoucna pravdìpodobnì vìt¹ina institucí pøejde na archivaci webových stránek do \texttt{warc} archivù. \texttt{Warc} tedy bude nejspí¹e oznaèen za jediný univerzálnì podporovaný formát. Jedná se o \texttt{ISO standard} a bude jím s nejvìt¹í pravdìpodobností i v budoucnosti.
...
\item Na rozdíl od \texttt{arc} archivù umo¾òují \texttt{warc} archivy ukládat nejen \texttt{HTTP} odezvy, ale také pùvodní po¾adavky.
...
\item Na rozdíl od \texttt{arc} formátu je mo¾né ukládat více druhù kontrolních souètù pro archivované soubory. To se dá jednodu¹e realizovat pomocí ¹kálovatelných polí pro metadata. \texttt{Arc} archivy, které pou¾ívá WebArchiv, provádí pouze \texttt{MD5} kontrolní souèty souborù. Na rozdíl \texttt{arc} archivù, je u \texttt{warc} archivu pou¾ít napøíklad i s \texttt{SHA-1} hashovací funkcí pro jednoznaèné urèení sta¾ených dokumentù.
\item Pøi naèítání libovolného souboru z archivu není potøeba rozbalovat celý archiv -- pøi 100 MB archivech jde o velkou re¾ii. Ka¾dý \texttt{warc} záznam má svùj vlastní \texttt{offset}, pøes který se dá k libovolnému souboru v archivu. Pak není potøeba soubor sekvenènì procházet, ale staèí pouze otevøít archiv a posunout ukazatel na správné místo. \cite{MASANES2006}
\end{itemize}
...
Jak jsem ji¾ zmínil vý¹e, s vyu¾itím formátu \texttt{warc} se v rámci projektu WebArchive poèítá, ale prozatím není provedeno dostateèné mno¾ství analýz. Nejdøíve bude potøeba poèkat na vyvinutí v¹ech potøebných nástrojù.
...
\label{warc:deduplikace}
\subsection{Deduplikace souborù}
Dùle¾itým aspektem standardu \texttt{warc} je mo¾nost deduplikace souborù. Jedná se o mo¾nost odstranìní tìch souborù, které se v repozitáøi objevují vícekrát.

Pokud se napøíklad v rùzných verzích urèitého sta¾eného zdroje nachází stejný soubor, je výhodné ho z dùvodu ¹etøení datové kapacity úlo¾i¹tì ponechat v archivu pouze jednou. \texttt{Deduplikace} souboru v¹ak není jednoduchá operace. Nese s sebou spoustu problémù, které je tøeba vyøe¹it.

Hlavním problémem je, ¾e takové duplicitní soubory se budou typicky nacházet v rùzných archivech. Proto je problematické pøi interpretaci urèité verze webového zdroje implementovat paralelní naèítání souboru z jiných archivù. Dále je problematické uchovávání konzistentních dat v jednotlivých verzích webových zdrojù -- ztráta jednoho archivu by mohla znamenat ztrátu dat pro více verzí webového zdroje.

Samostatným problémem je pøevedení starých archivù ve formátu \texttt{arc} do formátu \texttt{warc} a pøitom zajistit bezpeènou deduplikaci ulo¾ených dat. Napøíklad v British Library mo¾nost deduplikace pøi migraci svého archivu ani nevyu¾ili. \cite{PROKOP2012} Øe¹itelé projektu WebArchiv by v¹ak rádi této mo¾nosti vyu¾ili.

Mnou testované nástroje deduplikaci pøi migraci neumo¾òují, funkcionalita by se v¹ak dala implementovat. Nepodaøilo se mi najít zmínku o tom, ¾e by nìjaký volnì dostupný migraèní nástroj tuto mo¾nost vyu¾íval. Hlavním problémem pøi nasazení takového migraèního nástroje by bylo to, ¾e by takový migraèní nástroj nemusel splòovat v¹echny po¾adavky, které by na nì WebArchiv kladl. \footnote{O rozdílnosti pou¾ívaných nástrojù v rùzných projektech zabývajících se archivací se zabývám v jiné èásti této práce.} Bylo by proto pravdìpodobnì nutné, aby øe¾itelé takový nástroj sami implementovali nebo minimálnì upravili jiné øe¹ení.

Samotný formát \texttt{warc} nenabízí mo¾nost deduplikace, ale díky ¹iroké ¹kále metadat, které uchovává, je realizace deduplikace umo¾nìna mnohem lépe ne¾ u star¹ího formátu \texttt{arc}. \cite{Information and documentation -- The WARC File Format} \texttt{Deduplikace} je tedy øe¹ena na úrovni nástrojù, které \texttt{warc} soubory vytváøejí. Typickým pøíkladem nástroje, který deduplikuje sta¾ená data, je nástroj \texttt{Heritrix}. Ale existují i jiné nástroje, které tuto funkcionalitu implementují\footnote{Napøíklad nástroj \texttt{ArchiveIt}. Webové stránky nástroje \texttt{ArchiveIt} dostupné onlina na adrese \texttt{https://webarchive.jira.com/wiki/display/ARIH/Archive-It+Feature+Rele\\ase+3.0}}. Právì Heritrix je pou¾íván i ve WebArchivu. Problematikou deduplikace se zabývám v èásti vìnované danému nástoji \texttt{Heritrix na s. \pageref{nastroje:heritrix}} a v èásti, kde se vìnuji nástroji \texttt{Wayback na s. \pageref{nastroje:wayback}}.
